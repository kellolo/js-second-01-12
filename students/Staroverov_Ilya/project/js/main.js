...
class CatalogItem {
    ...
    render() {
        return `<div class="product-item" data-id="${this.idProduct}">
...
function renderCart() {
    ...
    document.querySelector(`.cart-block`).innerHTML = allProducts + priceCount();
}
...
function priceCount() {
    ...
    this.userCart.forEach(function (el) {
        TotalPriceCount += (el.price * el.quantity);
    });
    let totalRender = ` <div> <p class="product-title"> Total price: ${TotalPriceCount}</p></div> `;
    return totalRender;
}